        ...
        <Box sx={{
            ...
            <Stack spacing={2} sx={{
                ...
                <Box display={"flex"} justifyContent={"center"} alignItems={"center"} gap={1}>
                    <Header2 text={props.pool.users.length > 0 ? `${props.pool.users[0].user.display_name}'s pool` : "Create or join a pool"} color={"secondary.light"} />
                    {props.pool.share_code == null ? <DefaultButton text="share" action={handleShare}/> : <Header2 text={props.pool.share_code} color={"secondary.light"} />}
                </Box> : 
                ...
                <Grid item xs={2} display={"flex"} justifyContent={"center"} alignItems={"center"}>
                    ...
                    <AvatarGroup total={props.pool.users.length}>
                        {props.pool.users.map((user: PoolUser) => (
                            <Avatar key={user.user.display_name} alt={user.user.display_name} src={user.user.icon_url} />
                        ))}
                    </AvatarGroup> :
                    <>
                        {props.pool.users.map((user: PoolUser) => (
