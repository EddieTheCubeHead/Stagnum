...
@router.get("/")
def healthcheck(request_timer: RequestTimer, healthcheck_database_connection: HealthcheckDatabaseConnection,
    ...
    database_result = HealthcheckResourceResult(resource="database", status=database_status,
                                                time_elapsed=database_timer)
    return HealthcheckResult(status=database_result.status, time_elapsed=request_timer.get_elapsed_time(),
                             resources=[database_result])
