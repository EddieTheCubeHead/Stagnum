...
def should_have_required_scopes_in_login_redirect_response(test_server, validate_response, _required_headers,
...
def should_have_sixteen_bytes_of_noise_as_state_in_login_redirect_response(test_server, validate_response,
                                                                           _get_query_parameter):
    response = test_server.get("/auth/login")
    data_json = validate_response(response)
...
def should_have_random_state_in_login_redirect_response(test_server, validate_response, _get_query_parameter):
    responses = [test_server.get("/auth/login") for _ in range(10)]
