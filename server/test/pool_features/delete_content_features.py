...
def should_not_have_track_in_database_after_deletion(existing_pool: list[PoolMember], test_client: TestClient,
                                                     ...
                                                     db_connection: ConnectionManager) -> None:
...
def should_be_able_to_delete_separate_child_from_collection(create_pool: CreatePool,
                                                            ...
                                                            validate_response: ValidateResponse,
                                                            valid_token_header: Headers, test_client: TestClient) -> None:
    ...
    with db_connection.session() as session:
        all_tracks = session.scalars(select(PoolMember).where(
            and_(PoolMember.parent_id != None, PoolMember.user_id == logged_in_user_id))).unique().all()  # noqa: E711
...
def should_delete_all_children_on_parent_deletion(test_client: TestClient, validate_response: ValidateResponse,
    ...
    with db_connection.session() as session:
        all_tracks = session.scalars(select(PoolMember).where(
            and_(PoolMember.parent_id is not None, PoolMember.user_id == logged_in_user_id))).unique().all()
...
def should_return_error_if_member_is_not_users_own(test_client: TestClient, shared_pool_code: str,
                                                   ...
                                                   validate_response: ValidateResponse,
                                                   joined_user_header: Headers) -> None:
...
def should_include_token_in_headers(existing_pool: list[PoolMember], valid_token_header: Headers,
                                    test_client: TestClient, assert_token_in_headers: AssertTokenInHeaders) -> None:
    response = test_client.delete(f"/pool/content/{existing_pool[0].content_uri}", headers=valid_token_header)
